...
| Radar IPC | `src/main/ipc/radar.ts` | Registers `radar:*` channels (scan/read/setFields/task/handoff/init/inbox/config/pickFolder/openExternal/reveal/openInEditor) + pushes `radar:projects-changed`. `openExternal` allowlists http(s)/mailto (BLIP.md `links:` are untrusted); the editor launch never shell-parses a path. (The legacy ToDoPlus task store is fully deleted — `BLIP.md` is the only data model.) |
