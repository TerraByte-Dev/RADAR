...
| Logic | `src/renderer/src/lib/` | `radar.ts` (generic continuous time-scale math, unit-tested), `projectRadar.ts` (project→radar mapping), `selectors.ts` (views/calendar/logbook/heatmap/neglected), `nlp.ts` (quick-add parser), `date.ts`, `palette.ts`, `useKeyboard.ts` |
...
- **Radar:** `<canvas>` + rAF loop in `RadarView.tsx`. Distance = continuous log-compressed days-to-deadline (`projectRadiusFrac` → `radiusFracForDays`), falling back to the horizon band; rings NOW · 1 WEEK · 1 MONTH · 1 QUARTER + SOMEDAY. Angle = category sector, drag-pinned to a per-project `radar_angle` (visual only); same-sector/same-day blips **auto-fan** (`layoutBlipAngles`, fanning around pinned obstacles). Drag reschedules the **deadline** only when the drop changes day-bucket; a pure angular nudge just re-pins. Status visuals: blocked pulses, shipped dims, archived hidden, **ghost** (un-adopted repo) = faint dashed ring. Layout cached on a data signature, recomputed live only mid-drag. Honors `prefers-reduced-motion`.
